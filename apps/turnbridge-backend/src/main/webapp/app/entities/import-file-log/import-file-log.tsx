...
import { getEntities } from './import-file-log.reducer';

export const ImportFileLog = () => {
  ...
  const importFileLogList = useAppSelector(state => state.importFileLog.entities);
  const loading = useAppSelector(state => state.importFileLog.loading);
  const totalItems = useAppSelector(state => state.importFileLog.totalItems);
  ...
  return (
    <div>
      <h2 id="import-file-log-heading" data-cy="ImportFileLogHeading">
        <Translate contentKey="turnbridgeBackendApp.importFileLog.home.title">Import File Logs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="turnbridgeBackendApp.importFileLog.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/import-file-log/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="turnbridgeBackendApp.importFileLog.home.createLabel">Create new Import File Log</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {importFileLogList && importFileLogList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {importFileLogList.map((importFileLog, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/import-file-log/${importFileLog.id}`} color="link" size="sm">
                      {importFileLog.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/import-file-log/${importFileLog.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/import-file-log/${importFileLog.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/import-file-log/${importFileLog.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="turnbridgeBackendApp.importFileLog.home.notFound">No Import File Logs found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={importFileLogList && importFileLogList.length > 0 ? '' : 'd-none'}>
