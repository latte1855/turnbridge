...
public class WebhookDeliveryLogGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all webhookDeliveryLogs")
                    .get("/api/webhook-delivery-logs")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new webhookDeliveryLog")
                        .post("/api/webhook-delivery-logs")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"deliveryId\": \"SAMPLE_TEXT\"" +
                                ", \"event\": \"SAMPLE_TEXT\"" +
                                ", \"payload\": null" +
                                ", \"status\": \"SUCCESS\"" +
                                ", \"httpStatus\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_webhookDeliveryLog_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created webhookDeliveryLog").get("${new_webhookDeliveryLog_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created webhookDeliveryLog").delete("${new_webhookDeliveryLog_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WebhookDeliveryLog entity").exec(scn);
