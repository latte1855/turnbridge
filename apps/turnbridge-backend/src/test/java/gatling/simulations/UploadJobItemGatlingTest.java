...
public class UploadJobItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all uploadJobItems").get("/api/upload-job-items").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new uploadJobItem")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"traceId\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"QUEUED\"" +
                                ", \"resultCode\": \"SAMPLE_TEXT\"" +
                                ", \"resultMsg\": \"SAMPLE_TEXT\"" +
                                ", \"buyerId\": \"SAMPLE_TEXT\"" +
                                ", \"buyerName\": \"SAMPLE_TEXT\"" +
                                ...
                                ", \"assignedPrefix\": \"SAMPLE_TEXT\"" +
                                ", \"rawPayload\": null" +
                                ", \"rawHash\": \"SAMPLE_TEXT\"" +
                                ", \"profileDetected\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_uploadJobItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created uploadJobItem").get("${new_uploadJobItem_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created uploadJobItem").delete("${new_uploadJobItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UploadJobItem entity").exec(scn);
