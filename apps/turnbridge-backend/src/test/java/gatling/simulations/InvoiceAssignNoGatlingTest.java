...
public class InvoiceAssignNoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all invoiceAssignNos").get("/api/invoice-assign-nos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new invoiceAssignNo")
                        .post("/api/invoice-assign-nos")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_invoiceAssignNo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created invoiceAssignNo").get("${new_invoiceAssignNo_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created invoiceAssignNo").delete("${new_invoiceAssignNo_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InvoiceAssignNo entity").exec(scn);
