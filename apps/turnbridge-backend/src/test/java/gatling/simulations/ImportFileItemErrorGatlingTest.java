...
public class ImportFileItemErrorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all importFileItemErrors")
                    .get("/api/import-file-item-errors")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new importFileItemError")
                        .post("/api/import-file-item-errors")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_importFileItemError_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created importFileItemError").get("${new_importFileItemError_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created importFileItemError").delete("${new_importFileItemError_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ImportFileItemError entity").exec(scn);
