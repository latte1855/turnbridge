...
public class WebhookEndpointGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all webhookEndpoints").get("/api/webhook-endpoints").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new webhookEndpoint")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_webhookEndpoint_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created webhookEndpoint").get("${new_webhookEndpoint_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created webhookEndpoint").delete("${new_webhookEndpoint_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WebhookEndpoint entity").exec(scn);
