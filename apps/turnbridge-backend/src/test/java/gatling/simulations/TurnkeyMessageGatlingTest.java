...
public class TurnkeyMessageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all turnkeyMessages").get("/api/turnkey-messages").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new turnkeyMessage")
                        .post("/api/turnkey-messages")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_turnkeyMessage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created turnkeyMessage").get("${new_turnkeyMessage_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created turnkeyMessage").delete("${new_turnkeyMessage_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TurnkeyMessage entity").exec(scn);
