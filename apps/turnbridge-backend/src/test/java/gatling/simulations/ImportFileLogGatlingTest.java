...
public class ImportFileLogGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all importFileLogs").get("/api/import-file-logs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new importFileLog")
                        .post("/api/import-file-logs")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_importFileLog_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created importFileLog").get("${new_importFileLog_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created importFileLog").delete("${new_importFileLog_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ImportFileLog entity").exec(scn);
