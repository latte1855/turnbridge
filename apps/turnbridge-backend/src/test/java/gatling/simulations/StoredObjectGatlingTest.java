...
public class StoredObjectGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all storedObjects").get("/api/stored-objects").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new storedObject")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"bucket\": \"SAMPLE_TEXT\"" +
                                ", \"objectKey\": \"SAMPLE_TEXT\"" +
                                ", \"mediaType\": \"SAMPLE_TEXT\"" +
                                ", \"contentLength\": 0" +
                                ", \"sha256\": \"SAMPLE_TEXT\"" +
                                ", \"purpose\": \"UPLOAD_ORIGINAL\"" +
                                ", \"filename\": \"SAMPLE_TEXT\"" +
                                ", \"storageClass\": \"SAMPLE_TEXT\"" +
                                ", \"encryption\": \"SAMPLE_TEXT\"" +
                                ", \"metadata\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_storedObject_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created storedObject").get("${new_storedObject_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created storedObject").delete("${new_storedObject_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StoredObject entity").exec(scn);
