...
| 5 | 匯入明細「失敗筆數」欄呈現 ProblemDetail 而非筆數 | frontend | 1. 管理員或使用者開啟某批 import 2. 點「匯入明細」進入 detail 視圖 3. 第三欄顯示 `DATETIME_INVALID - 400 BAD_REQUEST ...` 等完整 ProblemDetail 字串 | 第三欄應顯示該筆的失敗數或匯總錯誤；ProblemDetail 應顯示在「欄位錯誤」/tooltip 以免干擾數值 | UI 目前把 `ProblemDetailWithCause` 直接顯示在第三欄，導致欄位標題與內容不符，`失敗筆數` 結果無法一眼辨識 | Evidence: 由 `Import Monitor` detail 查看該欄位 screenshot（可於 `docs/evidence/import-monitor-detail-problem.png` 取得) | Open | frontend | P2 | 需調整 `Import Monitor` UI，讓欄位映射到 `errorCount` / `fieldErrors` 並把 ProblemDetail 放入 tooltip/細節區。 |
| 6 | Turnkey 匯出/儀表板缺少事件過濾與搬移結果欄位 | frontend | 1. 進入 Portal「Turnkey 匯出」或「Webhook 儀表板」 2. 嘗試檢視 `XML_GENERATED`/`XML_DELIVERED_TO_TURNKEY`/`XML_DELIVERY_FAILURE` 事件 3. 缺乏 event code filter 與分頁，detail 也無 `turnkeyFile/reason` | UI 應提供事件過濾及分頁，並顯示搬移目的地（turnkeyFile）與失敗原因 | 目前無法在 UI 搜尋/過濾 Turnkey 搬移事件，只能看大量 ImportFileLog，Ops 無法快速得知搬移失敗或成功 | Evidence: Portal 截圖 `docs/evidence/turnkey-export-no-filter.png` | Open | frontend | P2 | 建議在 Turnkey 匯出/儀表板新增 event code filter、分頁、顯示 detail.turnkeyFile/reason。 |
