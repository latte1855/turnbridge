...
- 根據業主回覆與可維運性的考量，建議採用「**Backend 專注於檔案交換、驗證與轉檔**，**Agent 負責在客戶端收集/整理/產生合規檔案並上傳到 Backend**」的分工：
  - Agent（建議）職責：蒐集客戶端原始資料、進行本地前置檢核、將多來源資料整合為符合系統上傳規範的 CSV/ZIP（包含 MD5、metadata），並透過安全通道上傳至 Backend；對於沒有開發能力的客戶，可由業主提供 Agent（未來規劃）。
...
I. 例外與通知處理
- 若自動配號或匯入失敗（例如號碼不足、格式錯誤、MD5 錯誤），系統應註記錯誤並發出告警通知（Email / Web UI / Webhook），同時記錄 ImportFile 與 ImportFileLog 供後續人工處理。
